			...
			ULONG	*lpBytesReturned)
{
	...
	{
		DbgOut(("[WHD]ReadMsr Error Register:%x", *(ULONG*)lpInBuffer));
		*lpBytesReturned = 0;
			...
			ULONG	*lpBytesReturned)
{
	...
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
		DbgOut(("[WHD]WriteMsr Error Register:%x; value:%d", ((OLS_WRITE_MSR_INPUT*)lpInBuffer)->Register, ((OLS_WRITE_MSR_INPUT*)lpInBuffer)->Value));
		*lpBytesReturned = 0;
		return STATUS_UNSUCCESSFUL;
	}
}
			...
			ULONG	*lpBytesReturned)
{
	...
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
		DbgOut(("[WHD]ReadPmc Error performance counter:%x", *(ULONG*)lpInBuffer));
		*lpBytesReturned = 0;
		return STATUS_UNSUCCESSFUL;
	}
}
